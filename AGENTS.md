...
- `azure/` is an additive Microsoft-native companion distribution. It uses its own `azure/corpus/index.json` and plugin source root. Do not add Azure companion skills to `researcher/corpus/index.json`, `researcher/claims/index.jsonl`, `researcher/mechanisms/registry.jsonl`, or core activation fixtures unless a core `skills/` entry owns the change.
