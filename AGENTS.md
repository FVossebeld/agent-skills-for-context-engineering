...
- Repo health (`validate_repo.py`) and per-run readiness (`validate_run.py`) are different questions. CI runs `validate_repo.py --strict`, `azure/scripts/validate_azure.py`, `run_benchmarks.py`, and `check_activation_cases.py` on every PR via `.github/workflows/validate.yml`.
...
- The current published version is 2.3.0 across `.claude-plugin/marketplace.json`, `.plugin/plugin.json`, and root `SKILL.md`. The standard package includes 24 skills: 15 core skills plus 9 Azure adaptation skills.
- `azure/` is the Microsoft-native adaptation layer included in the default package. It uses its own `azure/corpus/index.json`; do not add Azure product-binding skills to `researcher/corpus/index.json`, `researcher/claims/index.jsonl`, `researcher/mechanisms/registry.jsonl`, or core activation fixtures unless a core `skills/` entry owns the change.
